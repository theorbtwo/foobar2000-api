...
namespace pfc {
  t_size base64_decode_estimate(const char * text) {
    ...
    if (textLen % 4 != 0) throw exception_invalid_params();
  ...
  void base64_decode(const char * text, void * out) {
    ...
    
    {
      ...
      for(; textWalk < max; textWalk ++) {
        ...
        if (v == 0xFF) throw exception_invalid_params();
      ...
      for(; textWalk < textLen; textWalk ++) {
        if (text[textWalk] != '=') throw exception_invalid_params();
      }
      memcpy(reinterpret_cast<t_uint8*>(out) + (outWritePtr/8), temp, tempWritePtr/8);
    }
  }
  void base64_encode(string_base & out, const void * in, t_size inSize) {
    out.reset(); base64_encode_append(out, in, inSize);
  }
  void base64_encode_append(string_base & out, const void * in, t_size inSize) {
    static const char alphabet[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    int shift = 0;
    int accum = 0;
